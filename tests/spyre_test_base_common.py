...
def _inject_extra_dtypes(test, extra_dtypes: set) -> object:
    """Return a deepcopy of *test* with *extra_dtypes* unioned into every
    @ops parametrizer's allowed_dtypes, so Spyre-specific dtype variants are
    generated by super().instantiate_test() without affecting other backends.

    If allowed_dtypes is None the test already runs for all supported dtypes
    and no injection is needed.
    """
    test_copy = copy.deepcopy(test)

    def _patch(parametrizer):
        if hasattr(parametrizer, "allowed_dtypes") and parametrizer.allowed_dtypes is not None:
            parametrizer.allowed_dtypes = set(parametrizer.allowed_dtypes) | extra_dtypes

    if hasattr(test_copy, "parametrize_fn"):
        _patch(test_copy.parametrize_fn)
    if hasattr(test_copy, "_parametrizers"):
        for p in test_copy._parametrizers:
            _patch(p)
