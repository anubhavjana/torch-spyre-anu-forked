...
class TestSpyreFuseNodesMaxFusedNodesCap(unittest.TestCase):
    """config.max_fused_nodes caps how many ops land in one bundle.

    Regression coverage for the crash where flash attention's multi-op tiled
    softmax, fused together with a neighboring RMSNorm+MLP, produced a
    bundle Deeptools' DDL dimension mapper couldn't solve (SIGABRT,
    "Could not find any suitable dimension mapping").
    """

    def test_uncapped_run_produces_one_bundle(self):
        """Sanity check: with no cap, all fusable nodes land in one bundle."""
        from torch_spyre._inductor import config

        sched = _make_scheduler()
        plains = [_make_snode(sched, _make_ir_op(), f"plain_{i}") for i in range(5)]
        with patch.object(config, "max_fused_nodes", None):
            result = spyre_fuse_nodes(plains)
        self.assertEqual(len(result), 1)

    def test_cap_splits_long_run_into_multiple_bundles(self):
        """A run longer than the cap is split into ceil(n / cap) bundles."""
        from torch._inductor.scheduler import FusedSchedulerNode
        from torch_spyre._inductor import config

        sched = _make_scheduler()
        plains = [_make_snode(sched, _make_ir_op(), f"plain_{i}") for i in range(5)]
        with patch.object(config, "max_fused_nodes", 2):
            result = spyre_fuse_nodes(plains)
        # 5 nodes capped at 2 per bundle -> bundles of size 2, 2, 1.
        self.assertEqual(len(result), 3)
        self.assertIsInstance(result[0], FusedSchedulerNode)
        self.assertIsInstance(result[1], FusedSchedulerNode)
        # Last (odd) node is returned as-is, not wrapped, per _make_fused.
        self.assertIs(result[2], plains[-1])

    def test_cap_of_one_disables_fusion(self):
        """max_fused_nodes=1 means every node is its own bundle."""
        from torch_spyre._inductor import config

        sched = _make_scheduler()
        plains = [_make_snode(sched, _make_ir_op(), f"plain_{i}") for i in range(3)]
        with patch.object(config, "max_fused_nodes", 1):
            result = spyre_fuse_nodes(plains)
        self.assertEqual(len(result), 3)
        for i, node in enumerate(result):
            self.assertIs(node, plains[i])

    def test_fallback_boundary_resets_cap_count(self):
        """A fallback boundary starts a fresh bundle count, not a continuation."""
        from torch._inductor.scheduler import ExternKernelSchedulerNode
        from torch_spyre._inductor import config
        ...
        fallback.scheduler = sched
        fallback.get_name.return_value = "fallback0"
        plain_c = _make_snode(sched, _make_ir_op(), "plain_c")
        plain_d = _make_snode(sched, _make_ir_op(), "plain_d")
        with patch.object(config, "max_fused_nodes", 2):
            result = spyre_fuse_nodes([plain_a, plain_b, fallback, plain_c, plain_d])
        # [plain_a, plain_b] bundle (cap=2) ; fallback ; [plain_c, plain_d] bundle (cap=2)
        self.assertEqual(len(result), 3)
        self.assertIs(result[1], fallback)
